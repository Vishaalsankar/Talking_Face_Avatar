...
class FaceAligner:
    ...
    def align(self, image, gray, rect, shape, scale=None):
        ...
        print(type(eyesCenter), eyesCenter)

        if eyesCenter is None or None in eyesCenter:
            raise ValueError("Eye landmarks not found.")

        # grab the rotation matrix for rotating and scaling the face
        eyesCenter = (float(eyesCenter[0]), float(eyesCenter[1]))  # Convert to float
        M = cv2.getRotationMatrix2D(eyesCenter, angle, scale)
        
        print("Updated Eyes Center:", eyesCenter, type(eyesCenter[0]), type(eyesCenter[1]))
